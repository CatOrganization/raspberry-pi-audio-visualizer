...
void vis_sound_wave_init(int screen_width, int screen_height, int audio_buffer_frames);
...
void vis_sound_wave_init(int width, int height, int audio_frames)
{
	vis_sound_wave_metadata.screen_width = width;
    vis_sound_wave_metadata.screen_height = height;
    vis_sound_wave_metadata.audio_buffer_frames = audio_frames;

	vis_sound_wave_metadata.sound_wave_line_points = malloc(sizeof(Vector2) * vis_sound_wave_metadata.audio_buffer_frames);
}

void vis_sound_wave_update(double *audio_frames)
{
	// Calculate sound wave line points and line thickness
	float horizontal_scale = vis_sound_wave_metadata.screen_width / vis_sound_wave_metadata.audio_buffer_frames;
	int center_y = vis_sound_wave_metadata.screen_height / 2;
	double max_y = -1;
	for (int n = 0; n < vis_sound_wave_metadata.audio_buffer_frames; n++)
	{
		vis_sound_wave_metadata.sound_wave_line_points[n].x = n * horizontal_scale;
		vis_sound_wave_metadata.sound_wave_line_points[n].y = center_y + (audio_frames[n] * vis_sound_wave_metadata.screen_height);
...
void vis_sound_wave_draw(bool verbose)
{
	...
	for (int n = 0; n < vis_sound_wave_metadata.audio_buffer_frames - 1; n++)
	{
